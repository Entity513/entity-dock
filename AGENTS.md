...
**アクセント色を全部の値に使わない。** 記録済みの値をすべて緑にすると
強調が均一化して序列が消え、ネオンだけが残って安く見える。

| 用途 | 色 |
|---|---|
| 値（体重・睡眠時間・kcal など） | `ink`（白系） |
| 進捗バーの充填、現在地、連続日数 | `accent` |
| PFC の3色 | `accent` / `warn` / `data` |
| 未記録 | `t-empty`（薄い `—`）。**警告色は使わない** |

文字サイズのクラス（`src/styles/index.css`）:
`t-display`（画面の主役）> `t-metric`（カードの主値）> `t-value`（行の値）
> `t-label`（項目名）> `t-sub`（補足）> `microlabel`（英字ラベル）

### 未記録の扱い

「未入力」と書かない。`—` を薄く出すだけにする。空欄に行動を要求する
ラベル（「＋ 記録」など）を全行に置かない。記録がない日ほど画面が
咎めてくる作りになり、続かなくなる。
